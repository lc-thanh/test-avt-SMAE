...
function downloadImage() {
    // Lấy ảnh từ canvas và hiển thị trong modal
...
function changeStep3() {
    ...
    var interval = setInterval(function() {
        ...
        if (progressValue >= 100) {
            ...
            // Hiển thị ảnh đã xử lý trực tiếp trên trang
            // var processedImage = document.getElementById("processed-image");
            var processedImage = $('#processed-image');
            if (downloadImageHd) {
                jpegDataURL = canvas2.toDataURL({
                    format: "png",
                    multiplier: 4 
                })
                console.log('jpegDataURL: ', jpegDataURL)
                processedImage.attr('src', jpegDataURL)
                // console.log('processedImage src: ', processedImage.attr('src'))
            } else {
                jpegDataURL = canvas2.toDataURL({
                    format: "png",
                    multiplier: 2
    ...
    // Không cần gọi formatImage và các xử lý liên quan đến server nữa
}
